...
ARM_VELOCITY_PLANNING ARM_RESET_MOTION = {0, 130, 1000, 1500, 500, 0.2, 0.2};  // 要做规划
ARM_VELOCITY_PLANNING ARM_SHOOT_MOTION = {ARM_INIT_POS, ARM_SHOOT_POS, 500, 800, 200, 0.2, 0.2};  // 要做规划
ARM_VELOCITY_PLANNING ARM_SHOOT_2_MOTION = {ARM_INIT_POS, -30, 500, 800, 500, 0.2, 0.2};  // 要做规划
...
void ad_plan_turnplate_motor_RPM(TURNPLATE_VELOCITY_PLANNING motion, float pos)
{
	...
	{
		TURN_MOTOR_TARGET_RPM = 0;  // 令夹爪不运动
		return;
	}
	...
	else
	{
		...
		// 规划RPM
		if     (S < Sac)       TURN_MOTOR_TARGET_RPM = sqrt(2.0f * Aac * S + motion.Vstart * motion.Vstart);               // 加速阶段
		else if(S < (Sac+Sco)) TURN_MOTOR_TARGET_RPM = motion.Vmax;                                                        // 匀速阶段
		else                   TURN_MOTOR_TARGET_RPM = sqrt(motion.Vend * motion.Vend - 2.0f * Ade * ABS(Ssu - S));  // 减速阶段
	}
	// 分配合适的正负号

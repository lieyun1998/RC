...
void USART6_IRQHandler(void)                	//串口6中断服务程序
{
  ...
  if(USART_GetITStatus(USART6,USART_IT_RXNE) != RESET) //中断产生 
	{
		USART_ClearITPendingBit(USART6,USART_IT_RXNE); //清除中断标志
		
		RES = USART_ReceiveData(USART6);     //接收串口6数据到buff缓冲区
		printf("%d\r\n",RES);   //这里我做打印数据处理
		
		Uart6_Buffer[Uart6_Rx] = USART_ReceiveData(USART6);     //接收串口1数据到buff缓冲区
		Uart6_Rx++; 
     		 
		if(Uart6_Buffer[Uart6_Rx-1] == 0x0a || Uart6_Rx == Max_BUFF_Len)    //如果接收到尾标识是换行符（或者等于最大接受数就清空重新接收）
		{
			if(Uart6_Buffer[0] == 0x11)                      //检测到头标识是我们需要的 
			{
				printf("%s\r\n",Uart6_Buffer);   //这里我做打印数据处理
				Uart6_Rx=0;                                   
			} 
			else
			{
				Uart6_Rx=0;                                   //不是我们需要的数据或者达到最大接收数则开始重新接收
			}
		}
	}
